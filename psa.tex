% Generated using the supplied basilisk_i18n style; content: Basilisk Systems P.S.A. agreement + comments/graphics; terminology: DIANA NATO, NATO, Unia Europejska.
...
% =========================================================
% TRYB DOKUMENTU
% ON  = wersja dla założycieli: komentarze, grafiki i oznaczenia robocze
% OFF = tekst dla prawnika lub notariusza z uwagami i pytaniami do konsultacji
% Zmień wyłącznie wartość ON/OFF w poniższej linii.
% =========================================================
\newcommand{\DlaZalozycieli}{OFF}

\newif\ifFoundersVersion
\ifdefstring{\DlaZalozycieli}{ON}{%
  \FoundersVersiontrue
}{%
  \ifdefstring{\DlaZalozycieli}{OFF}{%
    \FoundersVersionfalse
  }{%
    \PackageError{Basilisk Systems}{Nieprawidlowa wartosc DlaZalozycieli}%
      {Ustaw \string\DlaZalozycieli na ON albo OFF.}%
  }%
}
...
\ifFoundersVersion
  ...
  \BusinessPlanVersionText{Wersja założycielska}
\else
  \BusinessPlanAuthorsText{Basilisk Systems --- projekt do weryfikacji prawnej}
  \BusinessPlanConfidentialText{Projekt umowy --- do weryfikacji przez prawnika lub notariusza}
  \BusinessPlanFooterText{Basilisk Systems --- projekt umowy P.S.A.}
  \BusinessPlanVersionText{Wersja dla prawnika / notariusza}
\fi
...
\newcounter{paragraf}
\definecolor{RevisionBlue}{RGB}{22,55,96}
\definecolor{LegalViolet}{RGB}{92,62,130}
...
\newcommand{\ColorParagraph}[2]{%
  \refstepcounter{paragraf}%
  \phantomsection%
  \addcontentsline{toc}{section}{\S~\theparagraf\ #2}%
  \Needspace{6\baselineskip}%
  \subsection*{\textcolor{#1}{\S~\theparagraf\ #2}}%
}
...
\ifFoundersVersion
  ...
  \newenvironment{ChangedContractClause}[1]{%
    \begin{tcolorbox}[basiliskbox, colback=white, colframe=black!70, coltitle=white, fonttitle=\bfseries, title={TEKST UMOWY --- #1}]%
    \ColorParagraph{RevisionBlue}{#1}%
  }{%
    \end{tcolorbox}%
  }
  \newenvironment{UncertainContractClause}[1]{%
    \begin{tcolorbox}[basiliskbox, colback=white, colframe=black!70, coltitle=white, fonttitle=\bfseries, title={TEKST UMOWY --- #1}]%
    \ColorParagraph{LegalViolet}{#1}%
  }{%
    \end{tcolorbox}%
  }
\else
  % Wersja kancelaryjna: zwykły tekst prawny bez kolorowych ramek.
  \newenvironment{ContractClause}[1]{%
    \paragraf{#1}%
  }{%
    \par
...
\fi
\newenvironment{ChangedEnumerate}{%
  \begin{enumerate}[label=\textcolor{RevisionBlue}{\arabic*.},ref=\arabic*]%
}{\end{enumerate}}
\newenvironment{UncertainEnumerate}{%
  \begin{enumerate}[label=\textcolor{LegalViolet}{\arabic*.},ref=\arabic*]%
}{\end{enumerate}}

% Uprzejme uwagi robocze widoczne wyłącznie w wersji dla prawnika/notariusza.
\newcommand{\ConsultationBox}[1]{%
  \begin{tcolorbox}[
    enhanced,breakable,sharp corners=south,arc=0.8mm,boxrule=0.4pt,
    left=5pt,right=5pt,top=4pt,bottom=4pt,before skip=5pt,after skip=7pt,
    colback=black!2,colframe=black!35,coltitle=black,
    fonttitle=\bfseries\small,title={KWESTIA DO WERYFIKACJI PRAWNEJ}]
    \small #1%
  \end{tcolorbox}%
}
\newcommand{\CriticalConsultationBox}[1]{%
  \begin{tcolorbox}[
    enhanced,breakable,sharp corners=south,arc=0.8mm,boxrule=0.8pt,
    left=5pt,right=5pt,top=4pt,bottom=4pt,before skip=6pt,after skip=8pt,
    colback=red!3,colframe=red!60!black,coltitle=red!60!black,
    fonttitle=\bfseries\small,title={WAŻNA KWESTIA DO WERYFIKACJI PRAWNEJ}]
    \small #1%
  \end{tcolorbox}%
}
\ifFoundersVersion
  \newcommand{\LegalComment}[1]{}
  \newcommand{\CriticalLegalComment}[1]{}
\else
  \newcommand{\LegalComment}[1]{\ConsultationBox{#1}}
  \newcommand{\CriticalLegalComment}[1]{\CriticalConsultationBox{#1}}
\fi
...
\else
  % W wersji kancelaryjnej pola pozostają widoczne, ale bez czerwonego koloru.
...
\ifFoundersVersion
  \hypersetup{
    ...
    pdfauthor={Basilisk Systems --- projekt roboczy dla założycieli}
  }
\else
  \hypersetup{
    pdftitle={Basilisk Systems P.S.A. --- projekt umowy spółki},
    pdfauthor={Basilisk Systems --- projekt do weryfikacji prawnej}
  }
\fi

\BusinessPlanAuthorsText{Basilisk Systems --- projekt umowy do weryfikacji prawnej i notarialnej}
\BusinessPlanConfidentialText{Projekt umowy --- tekst normatywny z integralnymi Załącznikami nr 1--3}
\BusinessPlanFooterText{Basilisk Systems --- projekt umowy P.S.A.}
\BusinessPlanVersionText{Dokument I}
\hypersetup{pdftitle={Basilisk Systems P.S.A. --- projekt umowy spółki},pdfauthor={Basilisk Systems --- projekt do weryfikacji prawnej i notarialnej}}
\begin{document}
\BusinessPlanTitlePage
  {Basilisk Systems}
  {Dokument I}
  {Wersja dla prawnika lub notariusza}
  {Projekt umowy prostej spółki akcyjnej}
  {Tekst normatywny umowy z integralnymi Załącznikami nr 1--3}
  {1.0}
\tableofcontents
\newpage
...
\item \textbf{Podstawowy przedmiot działalności Spółki — 10 kodów przewidzianych do ujawnienia w KRS:}

\begin{enumerate}

\item \textbf{30.31.Z — Produkcja cywilnych statków powietrznych, statków kosmicznych i podobnych maszyn} — działalność przeważająca Spółki, obejmująca w szczególności produkcję bezzałogowych statków powietrznych, dronów oraz platform przeznaczonych do działania w autonomicznych rojach;

\item \textbf{71.12.B — Pozostała działalność w zakresie inżynierii i związane z nią doradztwo techniczne} — obejmująca w szczególności projektowanie mechaniczne, elektroniczne, elektryczne i systemowe, integrację platform autonomicznych oraz opracowywanie architektury systemów robotycznych;

\item \textbf{72.10.Z — Badania naukowe i prace rozwojowe w dziedzinie nauk przyrodniczych i technicznych} — obejmujące w szczególności badania i rozwój w obszarze robotyki, autonomii, systemów rojowych, sztucznej inteligencji, nawigacji, komunikacji i systemów bezzałogowych;

\item \textbf{62.10.B — Pozostała działalność w zakresie programowania} — obejmująca w szczególności oprogramowanie autonomii, sterowania, zarządzania flotą, planowania misji, koordynacji rojów, telemetrii oraz sztucznej inteligencji;

\item \textbf{28.99.Z — Produkcja pozostałych maszyn specjalnego przeznaczenia, gdzie indziej niesklasyfikowana} — obejmująca w szczególności mobilne roboty lądowe, roboty kołowe i gąsienicowe, autonomiczne platformy robocze, roboty inspekcyjne, logistyczne i specjalistyczne;

\item \textbf{26.51.Z — Produkcja instrumentów i przyrządów pomiarowych, kontrolnych i nawigacyjnych} — obejmująca w szczególności systemy nawigacyjne, kontrolne, telemetryczne, pomiarowe, autopiloty, układy lokalizacji oraz urządzenia służące do integracji i kontroli platform autonomicznych;

\item \textbf{33.20.Z — Instalowanie maszyn przemysłowych, sprzętu i wyposażenia} — obejmujące w szczególności instalowanie i uruchamianie stacji naziemnych, doków, systemów ładowania, infrastruktury operatorskiej, urządzeń komunikacyjnych i pozostałego wyposażenia systemów autonomicznych;

\item \textbf{33.16.Z — Naprawa i konserwacja cywilnych statków powietrznych i statków kosmicznych} — obejmująca w szczególności serwis, diagnostykę, konserwację, naprawę i modernizację cywilnych bezzałogowych statków powietrznych;

\item \textbf{30.11.Z — Produkcja cywilnych statków i konstrukcji pływających} — obejmująca w szczególności autonomiczne i zdalnie sterowane jednostki nawodne, bezzałogowe łodzie, platformy pływające oraz morskie systemy robotyczne;

\item \textbf{28.30.Z — Produkcja maszyn dla rolnictwa i leśnictwa} — obejmująca w szczególności autonomiczne maszyny rolnicze i leśne, roboty terenowe oraz platformy przeznaczone do automatyzacji prac rolniczych i leśnych.

\end{enumerate}

\item \textbf{Pozostały przedmiot działalności Spółki — dodatkowe kody PKD przewidziane dla dalszego rozwoju działalności:}

\begin{enumerate}[label=\alph*)]

\item \textbf{33.15.Z — Naprawa i konserwacja cywilnych statków i łodzi} — obejmująca w szczególności naprawę, konserwację i modernizację autonomicznych jednostek nawodnych, bezzałogowych łodzi i pozostałych cywilnych platform pływających;

\item \textbf{33.12.Z — Naprawa i konserwacja maszyn} — obejmująca w szczególności serwis, naprawę, diagnostykę i modernizację robotów lądowych, maszyn specjalnego przeznaczenia oraz autonomicznych platform roboczych;

\item \textbf{33.13.Z — Naprawa i konserwacja urządzeń elektronicznych i optycznych} — obejmująca w szczególności serwis urządzeń kontrolnych, sensorów, aparatury pokładowej, układów pomiarowych oraz elektroniki systemów autonomicznych;

\item \textbf{29.10.E — Produkcja pozostałych pojazdów silnikowych, z wyłączeniem motocykli} — obejmująca w szczególności autonomiczne pojazdy kołowe, pojazdy terenowe, platformy amfibijne i inne samobieżne pojazdy specjalnego przeznaczenia;

\item \textbf{63.10.D — Pozostała działalność usługowa w zakresie infrastruktury obliczeniowej, przetwarzania danych, zarządzania stronami internetowymi (hosting) i działalności powiązane} — obejmująca w szczególności przetwarzanie telemetrii, danych sensorycznych, zarządzanie flotą urządzeń, infrastrukturę obliczeniową oraz usługi związane z systemami autonomicznymi;

\item \textbf{30.32.Z — Produkcja wojskowych statków powietrznych, statków kosmicznych i podobnych maszyn} — obejmująca w przyszłości, po spełnieniu właściwych wymogów prawnych, produkcję bezzałogowych statków powietrznych i innych platform lotniczych przeznaczonych do zastosowań wojskowych;

\item \textbf{30.13.Z — Produkcja wojskowych okrętów i jednostek pływających} — obejmująca w przyszłości, po spełnieniu właściwych wymogów prawnych, autonomiczne i zdalnie sterowane jednostki pływające przeznaczone do zastosowań wojskowych;

\item \textbf{30.40.Z — Produkcja wojskowych pojazdów bojowych} — obejmująca w przyszłości, po spełnieniu właściwych wymogów prawnych, wojskowe autonomiczne i zdalnie sterowane platformy lądowe mające charakter pojazdów bojowych;

\item \textbf{33.18.Z — Naprawa i konserwacja wojskowych pojazdów bojowych, okrętów, łodzi, statków powietrznych i statków kosmicznych} — obejmująca w przyszłości, po spełnieniu właściwych wymogów prawnych, serwis i utrzymanie wojskowych platform bezzałogowych i autonomicznych.

\end{enumerate}

\item W ramach powyższych rodzajów działalności Spółka może prowadzić działalność dotyczącą w szczególności:

\begin{enumerate}[label=\alph*)]

\item bezzałogowych statków powietrznych, autonomicznych rojów dronów oraz systemów ich obsługi;

\item autonomii zespołowej, koordynacji wielu platform, rekonfiguracji roju, przydzielania zadań oraz zdecentralizowanego podejmowania decyzji;

\item oprogramowania dowodzenia, sterowania, koordynacji misji, zarządzania flotą oraz kontroli operatora nad systemami autonomicznymi;

\item systemów komunikacji, wymiany danych, synchronizacji i współdziałania platform w środowisku o ograniczonej lub zakłócanej łączności;

\item systemów percepcji, widzenia komputerowego, fuzji sensorów, sztucznej inteligencji brzegowej oraz autonomicznej interpretacji otoczenia;

\item nawigacji, lokalizacji i orientacji przestrzennej, w tym działania w środowiskach bez dostępu do globalnych systemów nawigacji satelitarnej;

\item symulacji, cyfrowych bliźniaków, generowania danych syntetycznych, testowania oraz walidacji systemów autonomicznych i rojowych;

\item mobilnych robotów lądowych, autonomicznych platform kołowych i gąsienicowych oraz bezzałogowych pojazdów lądowych;

\item autonomicznych i zdalnie sterowanych jednostek nawodnych oraz systemów ich obsługi;

\item autonomicznych maszyn i robotów dla rolnictwa, leśnictwa, przemysłu, infrastruktury i gospodarki terenowej;

\item autopilotów, komputerów pokładowych, systemów sterowania, układów monitorowania stanu platformy, systemów pomiarowych i nawigacyjnych;

\item infrastruktury obliczeniowej, telemetrii, przetwarzania danych oraz systemów zarządzania flotą urządzeń;

\item stacji naziemnych, doków, systemów automatycznego startu i lądowania, systemów ładowania oraz infrastruktury operatorskiej;

\item integracji komponentów, sensorów, elektroniki, napędów, modułów komunikacyjnych, komputerów pokładowych oraz wyposażenia zadaniowego dostarczanego przez podmioty trzecie;

\item diagnostyki, instalowania, uruchamiania, serwisowania, naprawy, konserwacji, modernizacji i przebudowy systemów autonomicznych;

\item prototypów, demonstratorów technologii, platform badawczych oraz krótkich serii przedprodukcyjnych;

\item zastosowań cywilnych, przemysłowych, infrastrukturalnych, ratowniczych, rolniczych, leśnych, morskich, bezpieczeństwa publicznego oraz ochrony infrastruktury krytycznej;

\item zastosowań obronnych i wojskowych, wyłącznie po uzyskaniu wymaganych koncesji, zezwoleń, certyfikatów lub innych wymaganych prawem uprawnień.
```
...
\item Działalność podlegająca koncesji, zezwoleniu, licencji, certyfikacji, kontroli eksportu albo innym szczególnym wymogom może być rozpoczęta dopiero po spełnieniu wymagań wynikających z właściwych przepisów prawa.
...
\begin{ChangedContractClause}{Program motywacyjny — pula 15\% po pełnym rozwodnieniu}
\label{art:program-motywacyjny}
\begin{ChangedEnumerate}
...
\end{ChangedEnumerate}
\end{ChangedContractClause}

\begin{ChangedContractClause}{Akcjonariusze Funkcjonalni i mechanizm zwrotnego zbycia akcji}
...
\begin{ChangedEnumerate}
  ...
  \item Akcje Akcjonariusza Funkcjonalnego wskazane w Załączniku nr 2 podlegają mechanizmowi stopniowego zwalniania akcji z obowiązku zbycia przez okres 48 miesięcy od daty rozpoczęcia wskazanej w tym załączniku, na następujących zasadach:
  \begin{enumerate}[label=\alph*)]
  ...
  \item Szczegółowy tryb wykonania obowiązku zbycia, w tym zdarzenie uruchamiające procedurę, sposób stwierdzenia Odejścia Usprawiedliwionego albo Odejścia Zawinionego, wskazanie Dopuszczalnego Nabywcy, ofertę sprzedaży, ograniczone pełnomocnictwo, terminy wykonania, sposób zapłaty i moment przeniesienia akcji, określa odrębna umowa wykonawcza podpisana przez każdego Akcjonariusza Funkcjonalnego najpóźniej w dniu objęcia mechanizmem. Zakres dokumentacji wykonawczej może zostać doprecyzowany w odrębnym dokumencie uzupełniającym, który nie stanowi części niniejszej umowy.
\end{ChangedEnumerate}
\end{ChangedContractClause}

\begin{ChangedContractClause}{Dopuszczalny Nabywca i Test Bezpieczeństwa Nabywcy}
\label{art:dopuszczalny-nabywca}
\begin{ChangedEnumerate}
  \item „Dopuszczalnym Nabywcą” jest osoba fizyczna, osoba prawna albo inna jednostka organizacyjna, która przeszła z wynikiem pozytywnym Test Bezpieczeństwa Nabywcy określony w niniejszym paragrafie oraz uzyskała wymaganą zgodę Spółki.
  \item Test Bezpieczeństwa Nabywcy obejmuje w szczególności ustalenie i ocenę:
  \begin{enumerate}[label=\alph*)]
    \item tożsamości, obywatelstwa, siedziby, rzeczywistego miejsca zarządzania i jurysdykcji nabywcy;
    \item pełnej struktury własności, kontroli oraz beneficjentów rzeczywistych;
    \item bezpośredniej lub pośredniej kontroli państwowej oraz powiązań z organami państwa, służbami lub siłami zbrojnymi;
    \item sankcji, embarg, ograniczeń eksportowych i innych zakazów dotyczących nabywcy, podmiotów kontrolujących, beneficjentów rzeczywistych, pośredników i źródła finansowania;
    \item źródła środków i sposobu finansowania transakcji;
    \item zakresu dostępu do Kluczowej Własności Intelektualnej, danych technicznych i informacji chronionych, jaki wynikałby z transakcji;
    \item wymogów wynikających z kontraktów, zezwoleń, programów Unii Europejskiej, NATO, DIANA NATO lub innych programów bezpieczeństwa i obronności;
    \item ryzyka obejścia ograniczeń przez podmioty pośrednie, instrumenty zamienne, powiernictwo, wspólne głosowanie albo inne konstrukcje zapewniające faktyczną kontrolę;
    \item całokształtu ryzyka dla bezpieczeństwa Spółki, technologii, kontraktów, zezwoleń i ciągłości działalności.
  \end{enumerate}
  \item Nabywca mający obywatelstwo, siedzibę, rzeczywiste centrum zarządzania lub podmiot kontrolujący poza Unią Europejską i NATO nie jest wyłącznie z tej przyczyny automatycznie wykluczony, lecz może zostać uznany za Dopuszczalnego Nabywcę jedynie po pozytywnym wyniku Testu Bezpieczeństwa Nabywcy i po uzyskaniu jednomyślnej zgody obejmującej 100\% wszystkich głosów w Spółce.
  \item Dopuszczalnym Nabywcą nie może być osoba lub podmiot objęty sankcjami, zakazem udostępniania środków gospodarczych albo innym bezwzględnie obowiązującym zakazem, ani osoba lub podmiot, którego udział powodowałby naruszenie prawa, zezwolenia, koncesji, obowiązku kontroli eksportu albo wiążącego wymogu bezpieczeństwa.
  \item Każda osoba zamierzająca nabyć prawa dotyczące akcji jest zobowiązana przedstawić Radzie Dyrektorów dokumenty i informacje konieczne do przeprowadzenia Testu Bezpieczeństwa Nabywcy, w tym dotyczące własności, kontroli, beneficjentów rzeczywistych, źródła środków i celu transakcji.
  \item Kryteria Testu Bezpieczeństwa Nabywcy stosuje się również do zastawu, użytkowania, powiernictwa, opcji, prawa do głosu, prawa do korzyści ekonomicznych, instrumentów zamiennych oraz innych konstrukcji, które mogą prowadzić do bezpośredniego albo pośredniego przejęcia kontroli lub korzyści z akcji.
  \item Złagodzenie Testu Bezpieczeństwa Nabywcy albo ustanowienie wyjątku od wymogu jednomyślnej zgody dla nabywcy związanego z jurysdykcją spoza Unii Europejskiej i NATO wymaga zmiany niniejszej umowy i nie może naruszać bezwzględnie obowiązujących przepisów bezpieczeństwa, sankcji ani kontroli eksportu.
\end{ChangedEnumerate}
\end{ChangedContractClause}

\begin{ChangedContractClause}{Ograniczenie zbywania akcji i zgoda Spółki}
\label{art:zbywanie}
\begin{ChangedEnumerate}
  ...
  \begin{enumerate}[label=\alph*)]
    ...
    \item zakończono Test Bezpieczeństwa Nabywcy, w tym weryfikację własności, kontroli, sankcji, źródła środków, konfliktu interesów i ryzyk dla bezpieczeństwa technologii;
  ...
  \item Zgoda nie jest wymagana dla zbycia dokonanego w wykonaniu prawa przymusowego współzbycia, obowiązkowego zbycia w ramach mechanizmu zwrotnego zbycia akcji ani przejścia akcji na Spółkę w przypadkach dopuszczonych prawem, pod warunkiem że ostateczny nabywca jest Dopuszczalnym Nabywcą.
  \item Naruszenie obowiązku uzyskania zgody albo wymogów dotyczących Dopuszczalnego Nabywcy stanowi istotne naruszenie niniejszej umowy. Zbywca i nabywca, w zakresie w jakim są związani niniejszą umową, są zobowiązani niezwłocznie usunąć skutki naruszenia, a jeżeli jest to prawnie dopuszczalne i konieczne — doprowadzić w terminie 60 dni do zbycia akcji na rzecz Dopuszczalnego Nabywcy. Postanowienie to nie przesądza sposobu działania podmiotu prowadzącego rejestr akcjonariuszy, który działa zgodnie z przepisami prawa.
\end{ChangedEnumerate}
\end{ChangedContractClause}
\begin{ChangedContractClause}{Okres Ograniczenia Zbywania Akcji Założycieli}
\label{art:okres-ograniczenia-zbywania}
\begin{ChangedEnumerate}
  \item Przez 24 miesiące od dnia wpisu Spółki do rejestru Założyciel nie może zbyć ani obciążyć akcji bez uprzedniej zgody Walnego Zgromadzenia podjętej większością 75\% Głosów Uprawnionych w Sprawie, przy czym Założyciel występujący o zgodę jest w tej sprawie wyłączony od głosowania.
...
\begin{ChangedContractClause}{Dziedziczenie akcji}
...
\begin{UncertainContractClause}{Ochrona przed wejściem małżonka lub byłego małżonka}
\label{art:malzonek}
\begin{UncertainEnumerate}
  \item Każdy akcjonariusz będący osobą fizyczną zobowiązuje się, w granicach prawnie dopuszczalnych, podejmować działania zmierzające do ochrony struktury własnościowej Spółki oraz niezwłocznie informować Spółkę o zmianie ustroju majątkowego lub innym zdarzeniu mogącym prowadzić do objęcia akcji wspólnością, podziałem majątku albo roszczeniem małżonka.
  \item Małżonek albo były małżonek może uzyskać i wykonywać prawa z akcji wyłącznie w zakresie zgodnym z bezwzględnie obowiązującymi przepisami prawa oraz, jeżeli wymagają tego postanowienia niniejszej umowy, po spełnieniu wymogów Dopuszczalnego Nabywcy i uzyskaniu zgody Spółki zgodnie z §~\ref{art:zbywanie}.
  \item Jeżeli na skutek podziału majątku, ugody lub orzeczenia akcje miałyby przypaść małżonkowi albo byłemu małżonkowi niespełniającemu wymogów, akcjonariusz jest zobowiązany, w granicach prawnie dopuszczalnych, dążyć w pierwszej kolejności do rozliczenia pieniężnego bez przenoszenia akcji, a jeżeli akcje skutecznie przejdą na osobę niespełniającą wymogów — zastosowanie mają postanowienia o obowiązku doprowadzenia do zgodności z §~\ref{art:zbywanie}.
...
\end{UncertainEnumerate}
\end{UncertainContractClause}

\begin{ChangedContractClause}{Wartość Godziwa i Dzień Wyceny}
\label{art:wycena}
\begin{ChangedEnumerate}
...
\begin{ChangedContractClause}{Reprezentacja Spółki}
...
\begin{ChangedEnumerate}
  ...
  \item W umowie między Spółką a dyrektorem oraz w sporze z nim Spółkę reprezentuje pełnomocnik powołany uchwałą akcjonariuszy, z zastrzeżeniem bezwzględnie obowiązujących przepisów prawa.
  \item W umowie oraz sporze między Spółką a dyrektorem wykonawczym Spółkę może reprezentować także dyrektor niewykonawczy działający na podstawie uchwały Rady Dyrektorów podjętej wyłącznie przez dyrektorów niewykonawczych, o ile spełnione są ustawowe warunki zastosowania tego trybu.
...
\begin{ChangedContractClause}{Walne Zgromadzenie i sposób obliczania głosów}
\label{art:wz}
\begin{ChangedEnumerate}
  ...
  \item Zasada z ust. 6 nie zmienia większości określonych w niniejszej umowie jako procent „wszystkich głosów w Spółce”; takie progi oblicza się od pełnej liczby głosów istniejących w Spółce, chyba że bezwzględnie obowiązujący przepis prawa stanowi inaczej.
\end{ChangedEnumerate}
\end{ChangedContractClause}

\begin{ChangedContractClause}{Sprawy Zastrzeżone dla Walnego Zgromadzenia}
...
\begin{ChangedEnumerate}
  ...
  \item Uchwały dotyczące uznania za Dopuszczalnego Nabywcę osoby lub podmiotu związanego z jurysdykcją spoza Unii Europejskiej i NATO, jak również złagodzenia §~\ref{art:dopuszczalny-nabywca}, wymagają jednomyślności obejmującej 100\% wszystkich głosów i nie mogą naruszać bezwzględnie obowiązujących przepisów bezpieczeństwa, sankcji ani kontroli eksportu.
...
\begin{enumerate}
  ...
  \item Żadne postanowienie niniejszej umowy nie zastępuje odrębnego dokumentu przeniesienia, gdy przepisy wymagają oznaczenia utworu, pól eksploatacji, formy pisemnej, wpisu do rejestru lub innej szczególnej czynności. Szczegółowy zakres Umowy Przeniesienia Praw Własności Intelektualnej określa odrębny dokument sporządzony w formie właściwej dla przenoszonych praw.
...
\ifFoundersVersion
\DiscussionComment{Prośba o konsultację — transakcje konfliktowe}{%
Klauzula wprowadza wewnętrzny próg 2\% oraz szczególną procedurę dla transakcji z podmiotem powiązanym z dyrektorem. Do konsultacji pozostaje wyłącznie zgodność tego progu oraz zasad ujawnienia konfliktu, wyłączenia, obliczania quorum i reprezentacji z bezwzględnie obowiązującymi przepisami prawa.
}
\fi

\begin{UncertainContractClause}{Zakaz konkurencji, konflikt interesów i transakcje z podmiotami powiązanymi}
...
\begin{UncertainEnumerate}
  ...
  \item „Podmiotem Powiązanym z Dyrektorem” jest podmiot, w którym dyrektor, jego małżonek, osoba bliska albo podmiot przez nich kontrolowany:
  \begin{enumerate}[label=\alph*)]
  ...
  \item Rada Dyrektorów podejmuje uchwałę o wyłączeniu dyrektora pozostającego w konflikcie oraz powierza prowadzenie sprawy innemu dyrektorowi wykonawczemu. Dyrektor pozostający w konflikcie nie jest uwzględniany przy ustalaniu większości wymaganej dla rozstrzygnięcia tej sprawy, w zakresie dopuszczalnym przez prawo.

  \item Zawarcie transakcji z Podmiotem Powiązanym z Dyrektorem wymaga uprzedniej uchwały Rady Dyrektorów podjętej wyłącznie przez dyrektorów niepozostających w konflikcie interesów.

  \item Uchwała, o której mowa w ust. 8, powinna potwierdzać, że:
  \begin{enumerate}[label=\alph*)]
  ...
  \item Jeżeli nie można zapewnić wymaganej liczby osób niepozostających w konflikcie interesów, transakcja nie może zostać zawarta do czasu zapewnienia prawidłowej reprezentacji, w szczególności przez powołanie dodatkowego dyrektora przez Walne Zgromadzenie.
\end{UncertainEnumerate}
\end{UncertainContractClause}
...
\begin{ChangedContractClause}{Rozwiązywanie Impasu Decyzyjnego}
\label{art:impas}
\begin{ChangedEnumerate}
...
\begin{ChangedContractClause}{Postanowienia końcowe}
\label{art:final}
\begin{ChangedEnumerate}
