...
\BusinessPlanVersionText{Wersja 0.9.4-B --- wariant produkcyjny}
...
\hypersetup{
  pdftitle={Basilisk Systems P.S.A. --- projekt umowy spółki --- wariant produkcyjny},
...
\BusinessPlanTitlePage
  {Basilisk Systems}
  {Wersja 0.9.4-B --- wariant produkcyjny}
  {Umowa P.S.A.}
...
\begin{enumerate}[resume]
  ...
  \item Zachowanie relacji 50/20/10/10/10 dotyczy udziału Założycieli Pierwotnych między sobą na dzień zawiązania Spółki. Każda późniejsza emisja akcji, w tym realizacja programu motywacyjnego serii P, emisja akcji dla nowego akcjonariusza albo runda finansowania, może proporcjonalnie rozwodnić ich udział, chyba że uchwała emisyjna stanowi inaczej.
  \item Wszystkie 1 000 000 akcji emitowanych przy zawiązaniu Spółki stanowią akcje serii A. Akcje programu motywacyjnego mogą być emitowane jako odrębna seria P; emisja serii P nie zmienia liczby akcji serii A.
\end{enumerate}
...
\begin{ContractClause}{Program motywacyjny — pula 5\% po pełnym rozwodnieniu}
\label{art:program-motywacyjny}
\begin{enumerate}
  \item Tworzy się docelową pulę programu motywacyjnego dla kluczowych pracowników, współpracowników, dyrektorów i doradców Spółki, odpowiadającą maksymalnie \textbf{52 632 (pięćdziesięciu dwóm tysiącom sześciuset trzydziestu dwóm)} akcji serii P.
  \item Przy założeniu wyemitowania wszystkich 52 632 akcji serii P i braku innych emisji, łączna liczba akcji po pełnym wykorzystaniu puli wyniesie 1 052 632, a pula programu motywacyjnego będzie odpowiadała 5\% po pełnym rozwodnieniu po zaokrągleniu do jednej dziesiątej punktu procentowego. Różnica od matematycznie dokładnych 5\% wynika wyłącznie z niepodzielności akcji.
  ...
  \item Zwiększenie puli ponad 52 632 akcji wymaga zmiany niniejszej umowy albo odrębnej podstawy emisyjnej przyjętej większością określoną dla Spraw Zastrzeżonych.
\end{enumerate}
\end{ContractClause}
...
\begin{enumerate}
  \item Na potrzeby niniejszego paragrafu „Akcjonariuszem Funkcjonalnym” jest Założyciel Pierwotny wskazany w Załączniku nr 2, który zobowiązał się do stałego, osobistego i istotnego zaangażowania operacyjnego w rozwój Spółki.
  \item Akcje Akcjonariusza Funkcjonalnego podlegają mechanizmowi stopniowego zwalniania akcji z obowiązku zbycia przez okres 48 miesięcy od daty rozpoczęcia wskazanej w Załączniku nr 2, na następujących zasadach:
  \begin{enumerate}[label=\alph*)]
...
\begin{enumerate}
  \item Założyciel Pierwotny przez 12 miesięcy od dnia wpisu Spółki do rejestru nie może zbyć ani obciążyć objętych tym statusem akcji bez uprzedniej zgody Walnego Zgromadzenia podjętej większością 75\% Głosów Uprawnionych w Sprawie, przy czym osoba występująca o zgodę jest w tej sprawie wyłączona od głosowania. Ograniczenie to stanowi ograniczenie rozporządzenia akcją w rozumieniu art.~300$^{39}$ §~1 KSH; na podstawie art.~300$^{39}$ §~2 KSH strony postanawiają, że w Okresie Ograniczenia Zbywania Akcji przepisy art.~300$^{39}$ §~3--6 KSH oraz mechanizm wskazania innego nabywcy z §~\ref{art:zbywanie} ust.~3 nie znajdują zastosowania, a odmowa zgody nie rodzi po stronie Spółki obowiązku wskazania nabywcy.
...
\begin{enumerate}
  \item Akcjonariusz albo grupa akcjonariuszy posiadająca łącznie co najmniej \textbf{90\% wszystkich głosów} może, w związku z wiążącą ofertą nabycia 100\% akcji Spółki przez niezależnego Dopuszczalnego Nabywcę, zobowiązać pozostałych akcjonariuszy do zbycia wszystkich ich akcji.
...
\begin{enumerate}
  ...
  \item Niezależny Ekspert uwzględnia w szczególności przychody, wynik EBITDA i przepływy pieniężne, portfel zamówień i kontrakty, aktywa produkcyjne, zapasy, certyfikaty i zdolności wytwórcze, portfel własności intelektualnej i poziom gotowości technologicznej, ryzyka regulacyjne, zadłużenie, środki pieniężne, perspektywy komercjalizacji oraz ostatnią rynkową transakcję na akcjach Spółki, jeżeli miała miejsce.
...
\begin{enumerate}
  ...
  \begin{enumerate}[label=\alph*)]
    ...
    \item przygotowanie i wykonanie emisji akcji serii P w zakresie powierzonym Radzie Dyrektorów uchwałą Walnego Zgromadzenia, bez kompetencji do samodzielnego podjęcia uchwały o emisji ani do samodzielnego pozbawienia prawa poboru.
  \end{enumerate}
...
\begin{enumerate}
  ...
  \begin{enumerate}[label=\alph*)]
    ...
    \item emisja akcji, warrantów, opcji, instrumentów zamiennych lub innych praw do akcji, w tym każda emisja akcji serii P;
...
\begin{enumerate}
  ...
  \item W pierwszych 36 miesiącach od wpisu Spółki do rejestru rekomendowanym sposobem wykorzystania zysku jest jego reinwestowanie. Walne Zgromadzenie może jednak postanowić o wypłacie dywidendy albo zaliczki na poczet dywidendy bezwzględną większością głosów oddanych, jeżeli spełnione są wymogi ust.~3, a wypłata nie narusza zobowiązań wynikających z umów o dotacje, granty i finansowanie.
...
Niniejszy załącznik dotyczy wyłącznie Założycieli Pierwotnych posiadających akcje serii A.
...
\begin{enumerate}
  \item \textbf{Prawo głosu i udziału w decyzjach właścicielskich.} Akcjonariusz wykonuje prawo głosu zgodnie z liczbą głosów przypadających na jego akcje oraz uczestniczy w podejmowaniu uchwał na Walnym Zgromadzeniu albo poza Walnym Zgromadzeniem, w tym przy wykorzystaniu środków komunikacji elektronicznej --- §~\ref{art:akcje}, §~\ref{art:wz}.
  \item \textbf{Prawo do udziału w zysku i majątku likwidacyjnym.} W zakresie wynikającym z rodzaju posiadanych akcji akcjonariusz uczestniczy w dywidendzie oraz w majątku pozostałym po likwidacji; wypłaty następują z zachowaniem ograniczeń ustawowych i zasad określonych w umowie --- §~\ref{art:akcje}, §~\ref{art:zysk} i §~\ref{art:likwidacja}.
...
\subsection*{5. Dodatkowe obowiązki Akcjonariusza Funkcjonalnego}
\begin{enumerate}
  \item \textbf{Mechanizm 48-miesięczny.} Akcje Akcjonariusza Funkcjonalnego podlegają przez 48 miesięcy stopniowemu zwalnianiu z obowiązku zbycia; zasady obejmują 12-miesięczny okres początkowy, Odejście Usprawiedliwione, Odejście Dobrowolne, Odejście Zawinione i przypadki obowiązkowego zbycia — §~\ref{art:zwrotne-zbycie}.
...
\subsection*{7. Seria F: świadomie pominięta w wariancie produkcyjnym}
\textbf{Aktualny tekst umowy:} §~\ref{art:cap-table} ust.~5, §~\ref{art:program-motywacyjny} i §~\ref{art:zwrotne-zbycie}.

\textbf{Wątpliwość.} Wcześniejsze wersje projektu przewidywały odrębną serię F dla osoby dołączającej po zawiązaniu Spółki jako późniejszy współzałożyciel, z wkładem w postaci pracy lub usług i własnym mechanizmem zwrotnego zbycia.

\textbf{Rozwiązanie.} W wariancie produkcyjnym osobnej serii F nie przewidziano. Nowy wspólnik wchodzi do Spółki w drodze zwykłej emisji akcji na podstawie §~\ref{art:walne-zastrzezone}, za wkład pieniężny albo niepieniężny, a jego prawa i obowiązki określają uchwała emisyjna i umowa objęcia akcji. Mechanizm zwrotnego zbycia z §~\ref{art:zwrotne-zbycie} obejmuje wyłącznie Założycieli Pierwotnych wskazanych w Załączniku nr 2. Seria P pozostaje jedyną odrębną serią akcji przewidzianą w umowie.
...
Jeżeli emisja wymaga zmiany umowy Spółki, zawiadomienie powinno również wskazywać dotychczasowe postanowienia i projektowane zmiany. Uchwałę dotyczącą zmiany umowy Spółki umieszcza się w protokole sporządzonym przez notariusza. Przy emisji serii P mieszczącej się w istniejącej podstawie z art. 300$^{103}$ KSH zachowanie przepisów o zmianie umowy Spółki nie jest wymagane wyłącznie z tego powodu.
...
\begin{enumerate}
  \item \textbf{Uchwała o emisji.} Każda emisja wymaga uchwały Walnego Zgromadzenia podjętej większością 75\% wszystkich głosów w Spółce zgodnie z §~\ref{art:walne-zastrzezone}. Dla serii P stosuje się §~\ref{art:program-motywacyjny}.
...
\subsection*{5. Dodatkowa ścieżka dla serii P}
...
\begin{enumerate}
  ...
  \item sprawdza się pozostałą niewykorzystaną część limitu 52 632 akcji serii P;
...
\subsection*{6. Prawo poboru --- wariant zachowany}
...
\subsection*{7. Umowa objęcia akcji}
...
\subsection*{8. Wniesienie wymaganej części wkładów i dokumenty Rady Dyrektorów}
...
\subsection*{9. Zgłoszenie emisji do KRS}
...
\subsection*{10. Moment powstania nowych akcji}
...
\subsection*{11. Rejestr akcjonariuszy i zamknięcie procesu}
...
\begin{enumerate}
  ...
  \item zaktualizować ewidencję pozostałego limitu serii P;
...
\subsection*{12. Skrócona checklista dokumentów}
...
\begin{enumerate}
  ...
  \item zaktualizowana tabela akcjonariatu, rejestr limitu serii P i dokumentacja korporacyjna.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item \textbf{emisja akcji} — stosuje się §~\ref{art:walne-zastrzezone}, a dla serii P §~\ref{art:program-motywacyjny}; praktyczna procedura znajduje się w Załączniku nr 6.
\end{enumerate}
...
\clearpage
\section*{Załącznik nr 8 — Akcjonariusz Funkcjonalny — wyjaśnienie kategorii}
\addcontentsline{toc}{section}{Załącznik nr 8 — Akcjonariusz Funkcjonalny — wyjaśnienie kategorii}
...
Pojęcie „Akcjonariusz Funkcjonalny” jest kategorią używaną w §~\ref{art:zwrotne-zbycie} i oznacza \textbf{Założyciela Pierwotnego objętego mechanizmem czasowym} --- osobę, która zawarła umowę przy zawiązaniu Spółki, objęła akcje serii A i została wskazana w Załączniku nr 2 jako osoba zobowiązana do stałego, osobistego i istotnego zaangażowania operacyjnego. Status Założyciela Pierwotnego wynika z §~\ref{art:cap-table}, a objęcie mechanizmem z §~\ref{art:zwrotne-zbycie} i Załącznika nr 2.

Mechanizm obejmuje wyłącznie Założycieli Pierwotnych wskazanych w Załączniku nr 2. Osoba, która obejmie akcje po zawiązaniu Spółki, wchodzi do niej w drodze zwykłej emisji na podstawie §~\ref{art:walne-zastrzezone} i nie staje się Akcjonariuszem Funkcjonalnym; objęcie takiej osoby mechanizmem zwrotnego zbycia wymagałoby zmiany umowy Spółki. Warunki jej zaangażowania i ewentualne ograniczenia dotyczące jej akcji określają uchwała emisyjna, umowa objęcia akcji i odrębne umowy.

\subsection*{2. Obowiązki Akcjonariusza Funkcjonalnego}

\begin{enumerate}
  \item \textbf{Osobiste i istotne zaangażowanie.} Akcjonariusz Funkcjonalny jest objęty kategorią funkcjonalną dlatego, że jego pozycja jest związana ze stałym, osobistym i istotnym zaangażowaniem w rozwój Spółki --- §~\ref{art:zwrotne-zbycie}.
  \item \textbf{Mechanizm 48-miesięczny.} Akcje Akcjonariusza Funkcjonalnego podlegają stopniowemu zwalnianiu z obowiązku zbycia przez 48 miesięcy, z 12-miesięcznym okresem początkowym oraz dalszym miesięcznym zwalnianiem --- §~\ref{art:zwrotne-zbycie}. Początek okresu wskazuje Załącznik nr 2.
  \item \textbf{Skutki ustania zaangażowania.} Akcjonariusz Funkcjonalny podlega definicjom Odejścia Usprawiedliwionego, Odejścia Dobrowolnego i Odejścia Zawinionego oraz odpowiadającym im zasadom pozostawienia lub obowiązkowego zbycia akcji --- §~\ref{art:zwrotne-zbycie}.
  ...
  \item \textbf{Ograniczenie zbywania.} Akcjonariusz Funkcjonalny podlega 12-miesięcznemu Okresowi Ograniczenia Zbywania Akcji, liczonemu od dnia wpisu Spółki do rejestru --- §~\ref{art:okres-ograniczenia-zbywania}.
  \item \textbf{Zakaz konkurencji i konflikt interesów.} Akcjonariusz Funkcjonalny podlega obowiązkom dotyczącym działalności konkurencyjnej, przejmowania szans biznesowych i konfliktu interesów w zakresie określonym w §~\ref{art:konkurencja}.
  \item \textbf{Poufność i bezpieczeństwo.} Akcjonariusz Funkcjonalny podlega obowiązkom poufności, bezpieczeństwa informacji, cyberbezpieczeństwa i zgłaszania incydentów określonym w §~\ref{art:security}.
  \item \textbf{Kontrola eksportu i sankcje.} Akcjonariusz Funkcjonalny przestrzega obowiązków wynikających z §~\ref{art:export}; określone naruszenia tych obowiązków mogą równocześnie stanowić Odejście Zawinione zgodnie z §~\ref{art:zwrotne-zbycie}.
  \item \textbf{Ogólne obowiązki akcjonariusza.} Niezależnie od reżimu funkcjonalnego Akcjonariusz Funkcjonalny pozostaje akcjonariuszem i podlega ogólnym zasadom dotyczącym Dopuszczalnego Nabywcy, zbywania akcji, prawa pierwszeństwa, przymusowego współzbycia, poufności oraz wykonania obowiązków --- w szczególności §~\ref{art:dopuszczalny-nabywca}, §~\ref{art:zbywanie}, §~\ref{art:pierwszenstwo}, §~\ref{art:przymusowe-wspolzbycie}, §~\ref{art:security} i §~\ref{art:breach}.
\end{enumerate}

\subsection*{3. Prawa i zabezpieczenia Akcjonariusza Funkcjonalnego}
...
\begin{enumerate}
  ...
  \item \textbf{Pozostałe prawa akcjonariusza.} Akcjonariusz Funkcjonalny korzysta również z praw wynikających z posiadania akcji i z ogólnych postanowień umowy, zestawionych informacyjnie w Załączniku nr 4, w szczególności z §~\ref{art:wz}, §~\ref{art:pierwszenstwo}, §~\ref{art:przylaczenie}, §~\ref{art:przymusowe-wspolzbycie}, §~\ref{art:zysk} i §~\ref{art:finansowanie}.
\end{enumerate}

\textbf{Reguła interpretacyjna.} Jeżeli umowa posługuje się pojęciem „Akcjonariusz Funkcjonalny”, należy stosować reżim z §~\ref{art:zwrotne-zbycie} do każdego Założyciela Pierwotnego wskazanego w Załączniku nr 2 wpisem „TAK”; wobec Założyciela Pierwotnego wskazanego wpisem „NIE” oraz wobec każdego innego akcjonariusza postanowienia adresowane do Akcjonariusza Funkcjonalnego nie znajdują zastosowania.
...
\textbf{A3.} Cel główny: projektowanie, rozwój, produkcja, integracja i komercjalizacja BSP; fizyczna sztuczna inteligencja i oprogramowanie autonomii jako zaplecze technologiczne produktów & §~\ref{art:cel} ust.~1--2 & \textbf{P} & Spółka definiuje się przez linię produktów i zdolności wytwórcze; ust.~2 potwierdza, że rozwój oprogramowania autonomii dla tych produktów mieści się w celu. Przestawienie osi działalności na technologię jako cel (profil S) wymagałoby zmiany umowy większością 75\% (§~\ref{art:walne-zastrzezone} ust.~1 lit.~a i~g). \\
...
\textbf{A7.} Działalność przeważająca 30.31.Z (produkcja cywilnych statków powietrznych); 72.10.Z na drugiej pozycji & §~\ref{art:pkd} ust.~5 & \textbf{P} & Kod przeważający produkcyjny odpowiada spółce wytwórczej; ma znaczenie dla statystyki publicznej, kwalifikacji w programach wsparcia i postrzegania przez zamawiających. Kod B+R na drugiej pozycji zachowuje dostęp do programów grantowych. Do potwierdzenia z doradcą przed wpisem do KRS (sekcja~6). \\
...
\textbf{B3.} Zastrzeżenie rozwodnienia przez serię P, emisję dla nowego akcjonariusza albo rundę finansowania & §~\ref{art:cap-table} ust.~4--5 & \textbf{U} & Potwierdza, że relacja 50/20/10/10/10 dotyczy dnia zawiązania i nie chroni przed rozwodnieniem przy późniejszych emisjach; w profilu P dotyczy głównie wejścia nowego wspólnika. \\
...
\textbf{B6.} Wyłączenie pracy i usług jako wkładu na akcje serii A & §~\ref{art:wklady} ust.~2 & \textbf{U} & Upraszcza wycenę i sytuację podatkową w obu profilach (Załącznik nr~10). Wkład w postaci pracy --- cecha P.S.A. typowa dla startupów --- nie jest w umowie przewidziany dla żadnej serii akcji. \\
...
\textbf{C8.} Prawo przymusowego współzbycia (drag-along): 90\% wszystkich głosów, wiążąca oferta na 100\% akcji, gwarancje dla mniejszości & §~\ref{art:przymusowe-wspolzbycie} & \textbf{S} & Służy przeprowadzeniu wyjścia (sprzedaży całej Spółki) mimo sprzeciwu mniejszości i jest oczekiwane przez inwestorów. W spółce produkcyjnej nastawionej na długoterminową własność jest nietypowe i może być postrzegane jako zagrożenie dla mniejszościowych Założycieli. Działa z mocy samej umowy; przy podziale 50/20/10/10/10 próg 90\% wymaga zgody Założycieli A i B oraz co najmniej dwóch z Założycieli C, D i E. \\
...
\textbf{C11.} Wartość Godziwa, Dzień Wyceny, Niezależny Ekspert & §~\ref{art:wycena} & \textbf{U} & Standard rozliczeń między wspólnikami jest uniwersalny, a katalog czynników wyceny (ust.~5) wymienia obok własności intelektualnej i gotowości technologicznej także przychody, EBITDA, portfel zamówień, aktywa produkcyjne, zapasy, certyfikaty i zdolności wytwórcze; zwrot „w szczególności” nie zamyka katalogu. \\
\midrule
...
\textbf{D6.} Przygotowanie i wykonanie emisji serii P powierzone Radzie & §~\ref{art:rada-zastrzezone} ust.~1 lit.~j & \textbf{S} & Obsługa mechanizmu programu motywacyjnego. \\
...
\textbf{D8.} Sprawy Zastrzeżone (75\% wszystkich głosów): zmiana umowy, połączenie, podział i likwidacja, sprzedaż przedsiębiorstwa albo aktywów ponad 25\%, Kluczowa Własność Intelektualna, zmiana profilu działalności, wynagrodzenia dyrektorów, akcje własne, zobowiązania poza budżetem ponad 500~000~zł, ugody dotyczące własności intelektualnej & §~\ref{art:walne-zastrzezone} ust.~1 lit.~a, d--k, ust.~2, 4 & \textbf{U} & Ochrona mniejszości i kontrola decyzji strategicznych są uniwersalne. W profilu P: lit.~g (rezygnacja z działalności badawczo-rozwojowej, strategia produktów podwójnego zastosowania) chroni tożsamość technologiczną, a próg 500~000~zł poza budżetem (lit.~j) wymaga budżetu obejmującego inwestycje produkcyjne i kredyt, aby nie blokować bieżących decyzji. \\
\textbf{D9.} Sprawy Zastrzeżone dotyczące emisji akcji i instrumentów, serii P, programu motywacyjnego i zatwierdzania Odejścia Usprawiedliwionego & §~\ref{art:walne-zastrzezone} ust.~1 lit.~b--c, l & \textbf{S} & Bramki decyzyjne dla mechanizmów startupowych; w profilu P pozostają niewykorzystane. \\
\midrule
...
\textbf{F3.} Zysk: rekomendacja reinwestycji przez 36 miesięcy; rezerwy na badania i rozwój, ochronę własności intelektualnej, cyberbezpieczeństwo, certyfikację i ryzyka eksportowe & §~\ref{art:zysk} & \textbf{U} & Reinwestycja jest naturalna w obu profilach we wczesnym okresie; w profilu P właściciele oczekują zwykle dywidendy wcześniej niż w S --- ust.~2 dopuszcza wypłatę dywidendy bezwzględną większością głosów po spełnieniu wymogów ust.~3. Rezerwy na certyfikację i ryzyka eksportowe (ust.~4) są elementem produkcyjnym. \\
\textbf{F4.} Rozwiązywanie Impasu Decyzyjnego: negocjacje, mediacja, Niezależny Ekspert, budżet kontynuacyjny & §~\ref{art:impas} & \textbf{U} & Nie zależy od profilu. \\
\textbf{F5.} Naruszenia i wykonanie obowiązków; pełnomocnictwa do obowiązkowego zbycia & §~\ref{art:breach} & \textbf{U} & Ust.~1--2 są uniwersalne; ust.~3 obsługuje mechanizm zwrotnego zbycia (S). \\
\textbf{F6.} Rozwiązanie i likwidacja: klasyfikacja bezpieczeństwa, eksportowa i archiwalna nośników, prototypów i dokumentacji; podział majątku z uwzględnieniem praw szczególnych & §~\ref{art:likwidacja} & \textbf{U} & Ust.~3 (klasyfikacja eksportowa prototypów i dokumentacji przy likwidacji) jest elementem produkcyjnym; zastrzeżenie „praw szczególnych” w ust.~4 przygotowuje miejsce na uprzywilejowanie likwidacyjne inwestora (S). \\
\textbf{F7.} Postanowienia końcowe: zmiana umowy, spory, status załączników & §~\ref{art:final} & \textbf{U} & Nie zależy od profilu. \\
\bottomrule
\end{longtable}
...
Nr 6 --- Proces emisji akcji & \textbf{S} & Checklista pod emisje serii P oraz emisje dla nowych akcjonariuszy; w profilu P wykorzystywana głównie przy emisji dla nowego wspólnika. \\
...
\subsection*{4. Elementy startupowe zachowane w wariancie produkcyjnym --- co działa od razu, a co dopiero po uchwale}

\textbf{Pominięte w wariancie produkcyjnym.} Odrębna seria akcji dla późniejszych współzałożycieli, odrębny tryb rundy inwestycyjnej z katalogiem zwyczajowych warunków inwestorskich (wraz z mapą term-sheetu) oraz startupowa hierarchia celu z kodem przeważającym 72.10.Z nie występują w niniejszym wariancie umowy; ich brzmienie zachowuje wariant inwestycyjny.

\textbf{Uśpione do czasu odrębnej decyzji.} Następujące elementy S nie wywołują skutków bez dodatkowej uchwały Walnego Zgromadzenia podjętej większością 75\% wszystkich głosów albo bez odpowiedniego wpisu w załączniku, więc nie generują kosztu ani ryzyka:
\begin{enumerate}
  \item program motywacyjny w zmniejszonej puli 5\% --- każda emisja serii P wymaga odrębnej uchwały (§~\ref{art:program-motywacyjny} ust.~3);
...
\textbf{Działające z mocy samej umowy.} Następujące elementy obowiązują od zawiązania Spółki niezależnie od dalszych uchwał:
\begin{enumerate}
  \item prawo przymusowego współzbycia z podwyższonym progiem --- może zostać uruchomione przez akcjonariuszy posiadających 90\% wszystkich głosów (§~\ref{art:przymusowe-wspolzbycie});
  \item wyłączenia spod zgody Spółki na rozporządzenie akcją (§~\ref{art:zbywanie} ust.~7) i wyjątek od Okresu Ograniczenia dla rundy finansowania zatwierdzonej większością 75\% (§~\ref{art:okres-ograniczenia-zbywania} ust.~2 lit.~a);
...
Rzeczywistego rozstrzygnięcia wymagają zatem tylko wpisy w Załączniku nr~2; pozostałe elementy S mogą pozostać w umowie jako opcja na przyszłość.
...
\begin{enumerate}
  ...
  \item \textbf{Progi kwotowe a budżet.} Finansowanie, zabezpieczenia i rozporządzenie aktywami ponad 100~000~zł zawsze wymagają uchwały Rady (§~\ref{art:rada-zastrzezone} ust.~1 lit.~d); wydatki ponad 250~000~zł i zobowiązania ponad 500~000~zł wymagają uchwał tylko poza zatwierdzonym budżetem (§~\ref{art:rada-zastrzezone} ust.~1 lit.~c; §~\ref{art:walne-zastrzezone} ust.~1 lit.~j). Roczny budżet powinien więc obejmować nakłady inwestycyjne, zapasy, leasing i kredyt.
...
\begin{enumerate}
  \item \textbf{Działalność przeważająca:} 30.31.Z przyjęto w tym wariancie jako kod przeważający (§~\ref{art:pkd} ust.~5) --- do potwierdzenia z doradcą przed wpisem do KRS (statystyka publiczna, programy wsparcia, sprawozdawczość).
  ...
  \item \textbf{Progi kwotowe i zakres budżetu:} próg 250~000~zł dla wydatków poza budżetem (§~\ref{art:rada-zastrzezone} ust.~1 lit.~c) przyjęto w tym wariancie; próg 100~000~zł dla finansowania i zabezpieczeń (lit.~d) oraz 500~000~zł na poziomie Walnego Zgromadzenia (§~\ref{art:walne-zastrzezone} ust.~1 lit.~j) pozostają do potwierdzenia wobec planowanej skali zakupów i leasingu.
