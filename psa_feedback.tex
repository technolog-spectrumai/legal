...
Tutaj wklejamy opinie kancelarii w kolejności otrzymania, oznaczone R1, R2, \ldots{} Każdy wpis zawiera: datę, zakres (A--E z zapytania), treść dosłowną albo załączone memorandum, a pod nią stanowisko i decyzje Założycieli co do poszczególnych uwag. Wcześniejszych wpisów nie edytujemy. Pytania przesłane kancelarii znajdują się w plikach \texttt{email.md} (treść maila z listą ogólną do wyceny) i \texttt{law.md} (pytania szczegółowe wysyłane razem z \texttt{psa.pdf} w drugim mailu, \texttt{mail.md}); odpowiedzi R\emph{n} odnosimy do numeracji pytań z \texttt{law.md}.
