...
\label{sha:status}
\begin{enumerate}
  \item Każdy Założyciel wskazany w Załączniku nr~2 do Umowy Spółki jako funkcjonalny potwierdza wpisane tam: liczbę Akcji Objętych Mechanizmem, datę rozpoczęcia oraz rolę i minimalne zaangażowanie.
...
\label{sha:ip}
\begin{enumerate}
  \item Każdy Założyciel potwierdza, że przeniósł na Spółkę prawa stanowiące jego wkład niepieniężny zgodnie z §~\ref{art:wklady} \US{} i Załącznikiem nr~1 do Umowy Spółki, na podstawie dokumentów rozporządzających, o których mowa w §~\ref{art:wklady} ust.~4 \US, oraz że oświadczenia złożone w §~\ref{art:wlasnosc-intelektualna} \US{} są prawdziwe i kompletne.
...
\begin{enumerate}
  ...
  \item Zawiadomienia składa się w formie dokumentowej na adres poczty elektronicznej Założyciela wpisany do rejestru akcjonariuszy (§~\ref{art:wz} ust.~3 \US), a wobec Spółki --- na adres wpisany do rejestru przedsiębiorców; do czasu wpisu Spółki do rejestru stosuje się adresy wskazane pod podpisami stron.
  \item Załączniki nr~A--C stanowią integralną część umowy. Umowę sporządzono w \field{liczba} jednobrzmiących egzemplarzach, po jednym dla każdej ze stron.
\end{enumerate}
\end{ContractClause}
...
\\[0.6cm]
\multicolumn{2}{@{}l}{\field{adresy poczty elektronicznej stron do zawiadomień do czasu wpisu Spółki do rejestru}} \\
\end{tabularx}
\end{center}
...
\newpage
\section*{Załącznik nr C — Wzór oświadczenia o przystąpieniu}
\addcontentsline{toc}{section}{Załącznik nr C — Wzór oświadczenia o przystąpieniu}
