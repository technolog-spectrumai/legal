% Generated using the supplied biznesplan_i18n style; content: SpectrumAi umowa sp. z o.o. + comments/graphics.

\documentclass[12pt,a4paper]{report}
\usepackage{../biznesplan_i18n}
...
\BusinessPlanCompany{SpectrumAi}
\BusinessPlanWatermarkText{SpectrumAi}
\BusinessPlanWatermarkColor{black!6}
\BusinessPlanWatermarkFontSize{38}{46}
\BusinessPlanWatermarkAngle{38}
\BusinessPlanAuthorsText{SpectrumAi --- dokument roboczy dla założycieli}
\BusinessPlanConfidentialText{Dokument roboczy --- komentarze nie stanowią tekstu umowy}
\BusinessPlanFooterText{SpectrumAi --- umowa sp. z o.o. / tekst umowy + komentarze}
...
\begin{tikzpicture}[>=Latex, node distance=0.85cm,
  flow/.style={draw, rounded corners, align=center, text width=0.72\linewidth, minimum height=0.92cm}]
  \node[flow, fill=black!5] (contract) {\textbf{TEKST UMOWY}\\paragrafy przeznaczone do wersji podpisywanej};
  \node[flow, fill=orange!10, below=of contract] (comment) {\textbf{KOMENTARZ DO DYSKUSJI}\\notatki robocze, nie są częścią umowy};
  \node[flow, fill=blue!7, below=of comment] (graphic) {\textbf{GRAFIKA DO DYSKUSJI}\\ilustracja decyzyjna, nie jest częścią umowy};
  \node[flow, fill=green!7, below=of graphic] (meeting) {\textbf{POSIEDZENIE ZAŁOŻYCIELSKIE}\\ustalenia, poprawki, wersja do podpisu};
  \draw[->] (contract) -- (comment);
  \draw[->] (comment) -- (graphic);
...
\begin{enumerate}
  \item \textbf{Nazwa Spółki brzmi:} SpectrumAi, spółka z ograniczoną odpowiedzialnością.
  \item Spółka może używać w obrocie skrótu: \textbf{SpectrumAi sp. z o.o.}
\end{enumerate}
\end{ContractClause}
...
\begin{itemize}[leftmargin=*]
  ...
  \item Do decyzji: czy nazwa \textbf{SpectrumAi} jest finalną nazwą do KRS oraz czy skrót \textbf{SpectrumAi sp. z o.o.} ma być stosowany w obrocie.
\end{itemize}
}
...
\centering
\begin{tikzpicture}[x=0.075\linewidth,y=0.72cm,
  bar/.style={draw, rounded corners=0.6pt},
  lab/.style={anchor=west, align=left, font=\small}]
  \foreach \y/\w/\shade/\name/\pct in {
    0/5.0/12/Wspólnik A/50\%,
    -1/2.0/22/Wspólnik B/20\%,
    -2/1.0/32/Wspólnik C/10\%,
    -3/1.0/42/Wspólnik D/10\%,
    -4/1.0/52/Wspólnik E/10\%}{
      \draw[bar, fill=black!\shade] (0,\y) rectangle (\w,\y+0.42);
      \node[lab] at (5.25,\y+0.21) {\name{} --- \pct};
  }
  \node[align=center, font=\small] at (3.2,-4.85) {Łącznie: 100 udziałów / 100\%};
\end{tikzpicture}
\end{DiscussionGraphic}
...
\begin{tikzpicture}[node distance=0.82cm, >=Latex,
  box/.style={draw, rounded corners, align=center, text width=0.72\linewidth, minimum height=0.88cm, fill=black!5}]
  \node[box] (w) {\textbf{Zgromadzenie Wspólników}\\uchwały, powołanie zarządu, decyzje właścicielskie};
  \node[box, below=of w] (z) {\textbf{Zarząd}\\prowadzenie spraw i reprezentacja};
  \node[box, below=of z] (sp) {\textbf{Spółka}\\operacje, IP, finanse, kontrakty};
  \node[box, fill=black!2, below=of sp] (rn) {\textbf{Rada Nadzorcza}\\opcjonalny nadzór, jeżeli zostanie ustanowiona};
  \draw[->] (w) -- node[right, font=\scriptsize]{powołuje / zatwierdza} (z);
  \draw[->] (z) -- node[right, font=\scriptsize]{zarządza} (sp);
  \draw[->] (rn) -- node[right, font=\scriptsize]{nadzór opcjonalny} (sp);
\end{tikzpicture}
\end{DiscussionGraphic}
...
\begin{tikzpicture}[>=Latex, node distance=0.75cm,
  vstep/.style={draw, rounded corners, align=center, text width=0.70\linewidth, minimum height=0.78cm, fill=black!5}]
  \node[vstep] (s0) {\textbf{Start}\\0\% udziałów nabytych};
  \node[vstep, below=of s0] (s6) {\textbf{Po 6 miesiącach}\\50\% udziałów nabytych};
  \node[vstep, below=of s6] (s12) {\textbf{Po 12 miesiącach}\\100\% udziałów nabytych};
  \draw[->] (s0) -- (s6);
  \draw[->] (s6) -- (s12);
\end{tikzpicture}
\end{DiscussionGraphic}
...
\centering
\begin{tikzpicture}[>=Latex, node distance=0.75cm,
  stage/.style={draw, rounded corners, align=center, fill=black!5, text width=0.70\linewidth, minimum height=0.78cm}]
  \node[stage] (offer) {\textbf{1. Oferta osoby trzeciej}\\cena, nabywca, warunki zapłaty};
  \node[stage, below=of offer] (pref) {\textbf{2. Prawo pierwszeństwa}\\pozostali wspólnicy mogą wejść w warunki oferty};
  \node[stage, below=of pref] (tag) {\textbf{3. Tag-along}\\mniejszość może przyłączyć się do sprzedaży};
  \node[stage, below=of tag] (drag) {\textbf{4. Drag-along}\\większość może wymusić współzbycie na tych samych warunkach};
  \draw[->] (offer) -- (pref);
  \draw[->] (pref) -- (tag);
  \draw[->] (tag) -- (drag);
\end{tikzpicture}
...
\begin{tikzpicture}[>=Latex, node distance=0.75cm,
  stage/.style={draw, rounded corners, align=center, fill=black!5, text width=0.70\linewidth, minimum height=0.78cm}]
  \node[stage] (n) {\textbf{Negocjacje}\\30 dni};
  \node[stage, below=of n] (m) {\textbf{Mediacja}\\do 60 dni};
  \node[stage, below=of m] (p) {\textbf{Propozycje rozwiązania}\\14 dni};
  \node[stage, below=of p] (d) {\textbf{Deadlock}\\decyzja według umowy};
  \draw[->] (n) -- (m);
  \draw[->] (m) -- (p);
  \draw[->] (p) -- (d);
\end{tikzpicture}
